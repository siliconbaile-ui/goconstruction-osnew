        ...
        <div className="px-5 pt-5 pb-4 flex-shrink-0" style={{ borderBottom: '1px solid #F0EEEA' }}>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              {movil && (
                <button onClick={onCerrar} aria-label="Cerrar panel"
                  className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: '#F1F0ED', color: '#6B7382' }}>
                  <X className="w-4 h-4" />
                </button>
              )}
            </div>
          ...
          <div className="grid grid-cols-2 gap-2 mt-4">
            {[
              ...
              <div key={k.l} className="rounded-xl px-4 py-3.5" style={{ background: '#F8F7F5' }}>
                <div className="text-2xl sm:text-xl font-semibold leading-none" style={{ color: k.c }}>{k.v}</div>
                <div className="text-[11px] mt-1.5" style={{ color: '#8A94A6' }}>{k.l}</div>
              </div>
            ))}
          </div>
        ...
        <div className="flex-1 overflow-y-auto min-h-0 p-4 space-y-2.5">
          {tab === 'alertas' && (
            topAlerts.length === 0 ? (
              ...
              <button key={a.id} onClick={() => onPrompt(`Revisa la alerta "${a.titulo}" y dime qué debo hacer`)}
                className="w-full text-left rounded-xl p-4 transition-colors hover:bg-gray-50 active:bg-gray-100"
                ...
                <div className="text-sm font-medium leading-snug" style={{ color: '#141821' }}>{a.titulo}</div>
                {a.mensaje && <div className="text-xs mt-1 line-clamp-2" style={{ color: '#8A94A6' }}>{a.mensaje}</div>}
              </button>
            ))
          )}
          ...
          {tab === 'acciones' && [
            ...
            <button key={a} onClick={() => onPrompt(a)}
              className="w-full text-left rounded-xl p-4 min-h-12 text-sm font-medium transition-colors hover:bg-gray-50 active:bg-gray-100 flex items-center gap-2"
            ...
            <>
              ...
              {loadingConvs ? (
                ...
                <div key={c.id} onClick={() => setActiveId(c.id)}
                  className="group flex items-center gap-2 rounded-xl px-3 py-3.5 min-h-12 cursor-pointer text-sm transition-colors"
                  style={activeId === c.id
                  ...
                  <button onClick={(e) => { e.stopPropagation(); onDelete(c.id); }}
                    className="p-2 -m-1 rounded-full sm:opacity-0 sm:group-hover:opacity-100" style={{ color: '#A8B0BF' }}>
