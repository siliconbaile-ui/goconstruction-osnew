...
export default function AuthLayout({ icon: Icon, title, subtitle, footer, children }) {
  return <main className="relative flex min-h-[100dvh] items-center justify-center overflow-hidden bg-surface-base px-4 py-8">
    ...
    <section role="dialog" aria-modal="true" aria-labelledby="auth-title" className="relative z-10 w-full max-w-md rounded-3xl border border-foreground/10 bg-surface/95 p-6 shadow-2xl backdrop-blur-xl sm:p-8">
      ...
      <p className="mt-6 flex items-center justify-center gap-2 border-t border-hairline pt-5 text-[10px] text-muted-foreground"><ShieldCheck className="h-3.5 w-3.5 text-ok" />Acceso seguro · demo con cuenta personal</p>
    </section>
  </main>;
}