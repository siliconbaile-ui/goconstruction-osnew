...
export default function KapsoPanel() {
  ...
  return <section id="kapso" className="rounded-xl border border-border bg-card p-5 space-y-5 text-card-foreground">
    <header className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><MessageSquare className="h-5 w-5 text-primary" /><div><h2 className="font-semibold">WhatsApp · Kapso</h2><p className="text-xs text-muted-foreground">Bandeja de administración · operación manual</p></div></div><Button type="button" variant="outline" aria-expanded={open} aria-controls="kapso-content" onClick={() => setOpen(v => !v)}>{open ? 'Cerrar bandeja' : 'Abrir Kapso'}</Button></header>
    ...
    <p className="rounded-lg border border-border bg-muted p-3 text-xs text-muted-foreground">GO todavía no responde automáticamente por Kapso. La asociación de fotos a obras y las alertas automáticas con plantillas están pendientes. Tu conexión actual de WhatsApp continúa sin cambios.</p>
