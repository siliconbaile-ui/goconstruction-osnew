  ...
  return (
    <div className="go-module-page p-4 lg:p-6 space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: 'TOTAL RDIs', value: rdis.length, color: 'hsl(var(--muted-foreground))' },
          { label: 'ABIERTOS', value: rdis.filter(r => r.estado === 'abierto').length, color: 'hsl(var(--primary))' },
          { label: 'VENCIDOS', value: vencidos, color: vencidos > 0 ? 'hsl(var(--danger))' : 'hsl(var(--ok))' },
          { label: 'SIN ASIGNAR', value: sinAsignar, color: sinAsignar > 0 ? 'hsl(var(--warn))' : 'hsl(var(--ok))' },
        ].map(s => (
      ...
      <div className="go-module-filters flex gap-2">
        ...
        <div className="ml-auto flex gap-1 rounded p-0.5" style={{ background: 'hsl(var(--surface-0))', border: '1px solid hsl(var(--surface-2))' }}>
          {[{ key: 'kanban', icon: LayoutGrid }, { key: 'lista', icon: List }].map(v => {
            ...
            return (
              <button key={v.key} onClick={() => setVista(v.key)} title={v.key}
                ...
                style={vista === v.key ? { background: 'hsl(var(--primary))', color: 'white' } : { color: 'hsl(var(--muted-foreground))' }}>
      ...
      {showForm && (
        <OrionCard className="p-5">
          <div className="font-mono text-xs mb-4 uppercase" style={{ color: 'hsl(var(--muted-foreground))' }}>NUEVO REQUERIMIENTO DE INFORMACIÓN</div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
            <div className="sm:col-span-2">
              <label className="text-xs font-mono mb-1 block" style={{ color: 'hsl(var(--muted-foreground))' }}>Título del RDI *</label>
              <input value={form.titulo} onChange={e => setForm(p => ({ ...p, titulo: e.target.value }))} className="w-full px-3 py-2 rounded text-sm text-white font-mono" style={{ background: 'hsl(var(--surface-0))', border: '1px solid hsl(var(--surface-2))' }} placeholder="Ej: Consulta especificación enfierradura Eje 4-J" />
            </div>
            {[
              ...
              <div key={f.key}>
                ...
                <input value={form[f.key]} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value }))} className="w-full px-3 py-2 rounded text-sm text-white font-mono" style={{ background: 'hsl(var(--surface-0))', border: '1px solid hsl(var(--surface-2))' }} />
              </div>
            ))}
            <div>
              <label className="text-xs font-mono mb-1 block" style={{ color: 'hsl(var(--muted-foreground))' }}>Prioridad</label>
              <select value={form.prioridad} onChange={e => setForm(p => ({ ...p, prioridad: e.target.value }))} className="w-full px-3 py-2 rounded text-sm text-white font-mono" style={{ background: 'hsl(var(--surface-0))', border: '1px solid hsl(var(--surface-2))' }}>
                {['baja', 'media', 'alta', 'critica'].map(p => <option key={p} value={p}>{p}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs font-mono mb-1 block" style={{ color: 'hsl(var(--muted-foreground))' }}>Categoría</label>
              <select value={form.categoria} onChange={e => setForm(p => ({ ...p, categoria: e.target.value }))} className="w-full px-3 py-2 rounded text-sm text-white font-mono" style={{ background: 'hsl(var(--surface-0))', border: '1px solid hsl(var(--surface-2))' }}>
                {['diseno', 'materiales', 'procedimiento', 'contrato', 'seguridad', 'otro'].map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs font-mono mb-1 block" style={{ color: 'hsl(var(--muted-foreground))' }}>Partida Asociada</label>
            ...
            <div>
              <label className="text-xs font-mono mb-1 block" style={{ color: 'hsl(var(--muted-foreground))' }}>Fecha Vencimiento</label>
              <input type="date" value={form.fecha_vencimiento} onChange={e => setForm(p => ({ ...p, fecha_vencimiento: e.target.value }))} className="w-full px-3 py-2 rounded text-sm text-white font-mono" style={{ background: 'hsl(var(--surface-0))', border: '1px solid hsl(var(--surface-2))' }} />
            </div>
            <div className="sm:col-span-2">
              <EvidenciaGeo
            ...
            <div className="sm:col-span-2">
              ...
              <textarea value={form.descripcion} onChange={e => setForm(p => ({ ...p, descripcion: e.target.value }))} rows={3} className="w-full px-3 py-2 rounded text-sm text-white font-mono resize-none" style={{ background: 'hsl(var(--surface-0))', border: '1px solid hsl(var(--surface-2))' }} placeholder="Describe el requerimiento técnico..." />
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={crearRDI} className="px-4 py-2 rounded text-sm font-medium text-white" style={{ background: 'hsl(var(--primary))' }}>Registrar RDI</button>
              ...
              <OrionCard key={rdi.id} className={`p-4 ${vencido ? '' : ''}`} style={vencido ? { borderColor: 'rgba(211,84,0,0.4)' } : {}}>
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div className="flex-1 min-w-0">
                    ...
                    {rdi.descripcion && <div className="text-xs mt-1 line-clamp-2" style={{ color: 'hsl(var(--muted-foreground))' }}>{rdi.descripcion}</div>}
                  </div>
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs font-mono" style={{ color: 'hsl(var(--muted-foreground))' }}>
                ...
                {rdi.evidencia_url && (
                  <a href={rdi.evidencia_url} target="_blank" rel="noreferrer" className="mt-3 inline-block">
                    <img src={rdi.evidencia_url} alt="Evidencia de terreno" className="h-24 rounded object-cover" style={{ border: '1px solid hsl(var(--surface-2))' }} />
                  </a>
                )}

                {rdi.respuesta && (
                  <div className="mt-3 p-3 rounded text-xs" style={{ background: 'hsl(var(--surface-0))', border: '1px solid hsl(var(--surface-2))' }}>
                    <div className="font-mono text-[10px] mb-1" style={{ color: 'hsl(var(--ok))' }}>RESPUESTA REGISTRADA:</div>
                  ...
                  <div className="mt-3 p-3 rounded text-xs" style={{ background: 'rgba(0,51,153,0.08)', border: '1px solid rgba(0,51,153,0.35)' }}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      {typeof rdi.confianza_ia === 'number' && (
                        <span className="font-mono text-[10px]" style={{ color: rdi.confianza_ia >= 70 ? 'hsl(var(--ok))' : 'hsl(var(--warn))' }}>
                ...
                {['abierto', 'en_revision'].includes(rdi.estado) && (
                  <div className="mt-3 pt-3 flex gap-2 flex-wrap" style={{ borderTop: '1px solid hsl(var(--surface-2))' }}>
                    <button onClick={() => setRespondiendo(rdi.id)} className="px-3 py-1.5 rounded text-xs font-mono text-white flex items-center gap-1" style={{ background: 'hsl(var(--primary))' }}>
                    ...
                    <button
                      ...
                      style={{ background: 'rgba(0,51,153,0.15)', border: '1px solid rgba(0,51,153,0.4)', color: 'hsl(var(--info))' }}
                    >
                ...
                {respondiendo === rdi.id && (
                  <div className="mt-3" style={{ borderTop: '1px solid hsl(var(--surface-2))', paddingTop: '12px' }}>
                    <textarea
                      value={respuesta}
                    ...
                    />
                    <div className="flex gap-2">
                      <button onClick={() => responder(rdi.id)} className="px-3 py-1.5 rounded text-xs font-mono text-white" style={{ background: 'hsl(var(--primary))' }}>Registrar Respuesta</button>
                      <button onClick={() => setRespondiendo(null)} className="px-3 py-1.5 rounded text-xs font-mono text-slate-400" style={{ background: 'hsl(var(--surface-0))', border: '1px solid hsl(var(--surface-2))' }}>Cancelar</button>
                    </div>
                  </div>
                )}
              </OrionCard>
