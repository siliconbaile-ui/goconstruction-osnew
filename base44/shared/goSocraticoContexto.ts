...
export async function prepararSocraticoGo(base44, auditoria, registro, texto) {
  const scope = alcanceSocraticoGo(auditoria), db = base44.entities.ConocimientoObraGO, registros = [];
  ...
  if (retiro || (directo && pregunta && (confirmarGo(texto) || negarGo(texto)))) {
    permiso = await guardarSocraticoGo(base44, auditoria, 'consentimiento', {
      ...
      audiencia: 'administradores_desarrollo', finalidad: 'retomar_relato_y_validar_reglas', auditoria_conservada: true
    }, { fuente: texto });
  }
  ...
  const contexto = { socratico_habilitado: true, entorno: 'dev', simulado: true, first_value: valor?.id || null,
    ...
    audiencia: 'solo administradores de desarrollo', vinculacion_privada: false };
