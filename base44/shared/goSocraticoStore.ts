...
export const PERMISO_RELATO_GO = 'Guardaré tu relato y sus reglas para retomarlos; solo los administradores podrán verlos, junto con la auditoría. ¿Autorizas guardarlos?';
...
export function alcanceSocraticoGo(auditoria) {
  ...
  return { entorno: t.entorno, grupo_prueba: t.grupo_prueba, persona_id: t.persona_id };
}
export async function guardarSocraticoGo(base44, auditoria, tipo, datos, opciones = {}) {
  ...
  const db = base44.asServiceRole.entities.ConocimientoObraGO;
  ...
  const registro = previo || await db.create({ ...scope, clave, tipo, estado: opciones.estado || 'registrado',
    ...
    validador_persona_id: opciones.validador || '', dia_captura: opciones.dia || '', simulado: t.simulado, identidad_verificada: false });
...
export async function exigirReglaAplicableGo(base44, auditoria, id) {
  const regla = await unoAuditoriaGo(base44.asServiceRole.entities.ConocimientoObraGO, { ...alcanceSocraticoGo(auditoria), id, tipo: 'regla_obra' });
  const permisos = await base44.asServiceRole.entities.ConocimientoObraGO.filter({ ...alcanceSocraticoGo(auditoria), tipo: 'consentimiento' }, '-created_date', 1);
