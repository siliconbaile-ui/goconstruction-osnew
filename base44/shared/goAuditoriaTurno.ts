...
export async function iniciarAuditoriaGo(base44, mensaje, grupo, sesion, entorno = 'dev') {
  ...
  const contactoClave = await firmaOpacaGo(`contacto:${entorno}:${grupo}:${mensaje.phone_number_id}:${mensaje.remite_numero}`);
  const personas = base44.asServiceRole.entities.ContactoAuditoriaGO, turnos = base44.asServiceRole.entities.TurnoAuditoriaGO;
  const filtro = { contacto_clave: contactoClave, entorno, grupo_prueba: grupo };
  const simulado = entorno === 'dev';
  ...
  const clave = await hashGo(`${entorno}:${grupo}:${mensaje.phone_number_id}:${mensaje.message_id}`);
  ...
  const previos = await turnos.filter({ persona_id: persona.id, sesion_id: sesion, entorno, grupo_prueba: grupo, estado: 'completado' }, '-created_date', 2);
  const anterior = previos.find(t => t.wamid !== mensaje.message_id) || null;
  if (!turno) turno = await turnos.create({ clave, entorno, grupo_prueba: grupo, sesion_id: sesion, persona_id: persona.id,
    ...
    estado_anterior: anterior?.estado_siguiente || 'sin_iniciar', simulado });
  ...
  if (turno.estado === 'completado') {
    await registrar('reintento', { resultado: 'respuesta_persistida_reutilizada', envio_real: !simulado });
  ...
  if (turno.estado === 'procesando') {
    await registrar('reintento', { resultado: 'en_proceso_no_reinvocar', envio_real: !simulado });
  ...
  const auditoria = { turno, persona, anterior, entorno, simulado, registrar,
