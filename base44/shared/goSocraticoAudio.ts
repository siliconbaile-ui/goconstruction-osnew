...
export async function guardarAudioSimuladoGo(base44, auditoria, entrada) {
  if (entrada.tipo_mensaje !== 'audio') return;
  await guardarSocraticoGo(base44, auditoria, 'evidencia', { medio: 'audio_simulado_transcrito', direccion: 'entrante',
    transcripcion: entrada.transcripcion, audio_recibido: false, archivo_uri: null,
    origen: 'fixture_administrativo', no_es_transcripcion_de_audio_real: true }, { paso: 'audio_entrada', fuente: entrada.transcripcion });
}
export async function respuestaAudioDesarrolloGo(base44, auditoria, entrada, resultado) {
  ...
  const previo = await unoAuditoriaGo(base44.entities.ConocimientoObraGO, { ...alcanceSocraticoGo(auditoria), clave });
  ...
  try {
    ...
    const { file_uri } = await base44.integrations.Core.UploadPrivateFile({ file: new File([bytes], 'go-respuesta.mp3', { type: 'audio/mpeg' }) });
    const datos = { direccion: 'saliente', archivo_uri: file_uri, mime_type: 'audio/mpeg', texto: resultado.respuesta,
      motor: voz.data.motor, generado: true, enviado: false, simulado: true, nota_voz_whatsapp: false,
    ...
    return { ...resultado, audio: { generado: false, enviado: false, simulado: true, error: error.message } };
  }
}