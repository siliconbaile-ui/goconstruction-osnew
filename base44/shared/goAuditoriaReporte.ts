...
export async function exportarAuditoriaGo(base44, grupo, sesion = '') {
  const filtro = { entorno: 'dev', grupo_prueba: grupo };
  const registros = Object.fromEntries(await Promise.all(ENTIDADES.map(async nombre => [nombre, await leerTodoGo(base44.entities[nombre], filtro)])));
  ...
  const informe = { generado: new Date().toISOString(), entorno: 'dev', grupo_id: grupo, sesion_id: sesion || null,
    envio_whatsapp: false, wamid_sintetico: true, transcripciones, registros, integridad,
  ...
  const { file_uri } = await base44.integrations.Core.UploadPrivateFile({ file: archivo });
  const { signed_url } = await base44.integrations.Core.CreateFileSignedUrl({ file_uri, expires_in: 3600 });
  return { archivo_completo: signed_url, file_uri, data_env: 'dev', grupo_id: grupo, envio_whatsapp: false,
